...
// cycle limitations).
const unsigned TX_INTERVAL = 450;
...
void sensor_data() {
  ...
  //Temperatur
  float temperatureReadout = (bme.readTemperature() + 50) * 100; //Temperature is shifted 50°C to the right that temperatures are measured from -50°C to +50°C, then multiplied by 100 to avoid decimal places.
  uint16_t temperature = (uint16_t) temperatureReadout; //Cast temperature, float to uint16_t
  uint8_t hightempratureByte = (uint8_t) ((temperature & 0xFF00) >> 8); // bitshift, 8 bits to the right
  uint8_t lowtempratureByte = (uint8_t) (temperature & 0xFF); //bitshift, 8 bits to the left
